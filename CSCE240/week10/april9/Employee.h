...
class Employee {
 ...
 public:
  ...
  // ANY time we have a virtual function in our class, we should also include
  // a virtual destructor even if that function is empty in order to ensure
  // that the correct destructors are called whenever memory for a base class
  // pointer/reference is released
  virtual ~Employee() {
