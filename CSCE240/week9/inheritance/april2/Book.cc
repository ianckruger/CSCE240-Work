...
void Book::InsertChapter(int position, string chapter_name) {
  if ( position >= 1 && position <= _number_of_chapters + 1 ) {
    // increase the number of chapters by 1
    ++_number_of_chapters;
    // make a new block for the chapter titles and copy the old values
    // over with the new title in the correct spot
    // new block of the correct size
    string * temp = new string[_number_of_chapters];
    for ( int i = 0, j = 0; i < _number_of_chapters; ++i ) {
      if ( i == position - 1 )  // i = the spot for the new chapter title
        temp[i] = chapter_name;
      else
        temp[i] = _chapter_title[j++];
    }
    delete [] _chapter_title;  // free up the old, smaller block
    _chapter_title = temp;
  }
}
